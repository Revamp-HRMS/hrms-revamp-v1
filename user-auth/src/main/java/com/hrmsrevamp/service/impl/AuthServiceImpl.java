...
import com.hrmsrevamp.constants.UserStatusEnum;
...
import com.hrmsrevamp.model.JwtAuthenticationResponse;
...
public class AuthServiceImpl implements AuthService {
  ...
  @Override
  @Transactional
  public CustomResponse adminSignup(SignUpRequest request) {
    ...
    userRepository.save(userAuthHelper.setUserId(user));
  ...
  public CustomResponse signIn(SignInRequest request) {
    ...
    if ((Objects.nonNull(email) && !email.isBlank())
      ...
      return signInAdminFromBackend(email, password);
    } else if (Objects.nonNull(request.getUid()) && !request.getUid().isBlank()) {
      LOGGER.info("User signing by firebase.");
      return null;
    } else if (Objects.nonNull(request.getIdToken()) && !request.getIdToken().isBlank()) {
      LOGGER.info("User signing by firebase.");
      return null;
    } else {
  ...
  private CustomResponse signInAdminFromBackend(String email, String password) {
